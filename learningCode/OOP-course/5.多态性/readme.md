...
---
**例题：**  
友元运算符函数和成员运算符函数有什么不同？  

(1) 对双目运算符而言,成员运算符重载函数参数表中含有一个参数，而友元运算符重载函数参数表总含有两个参数；对于单目运算符而言，成员运算符重载函数参数表中没有参数，而友元运算符重载函数参数表中含有一个参数。  
(2) 双目运算符一般可以被重载为友元函数运算符重载函数或成员运算符重载函数，但有一些情况必须使用友元运算符重载函数，例如一个常数与一个对象相加。有的运算符（如`=` 等）只能使用成员运算符重载函数。  
...
对于类 Counter 实现后置自增运算符 c++，要求返回自增前的值，且支持链式操作（如 `(c++).print()`），以下哪个声明和实现是正确的？  
A:
``` cpp
...
答案： A。  
首先，后置 ++ 通过一个虚拟的 int 函数与前置区分。  
必须返回自增前的对象副本（**不能返回局部变量引用**），因此返回类型应该会 Counter （值）。选项 B 返回的是 temp 的引用，temp 是局部变量，函数结束就销毁了，会造成悬空引用。  

---
有关运算符重载正确的描述是：  
A. C++ 允许在重载运算符时改变运算符的优先级  
B. C++ 允许在重载运算符时改变运算符的操作个数  
C. C++ 允许在重载运算符时改变运算符的结合性  
D. C++ 允许在重载运算符时改变运算符原本的功能
...
**构造函数不能是虚函数**：因为构造函数在编译时静态绑定，无法实现运行时多态。  
析构函数可以是虚函数，且基类声明虚机构函数能确保对象
...
纯虚函数的作用是在基类中为其派生类保留一个函数的名字，以便派生类根据需要对它进行重新定义。纯虚函数没有函数体，他最后面的 `=0` 并不表示函数值为 0，而只起形式上的作用，告诉编译系统这是纯虚函数，纯虚函数不具备函数的功能，不能被调用。  

如果一个至少含有一个纯虚函数，那么这个类就被称为抽象类。  
## 应用
