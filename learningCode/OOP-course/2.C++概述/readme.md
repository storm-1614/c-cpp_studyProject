...
const 修饰的变量必须在定义的时候初始化，因为 const 常量一旦创建之后就不能被修改。  
...
从右往左读， name 是一个 const 指针，指向 const char。  

指向的数据是 const (const char)，不能通过 `name` 修改字符内容。指针本身是 const (`* const`) 不能让 `name` 指向别的地址。而且 "chen" 是字符串字面量存储在只读内存区。  
...
下面语句中正确的是：
A. `name[3]='a';` B. `name="lin";` C. `name= new char[5];` D. `name = new char('q');`

答案：A。

同样从右往左读， name 是一个 const 指针，指向 char。  
指针不能修改，但是指针指向的数据可以修改，所以选 A。  
...
函数重载的关键在于参数列表的不同，而返回类型不会被编译器用作函数重载的依据。  
## 作用域运算符 `::`
...
`(初始值)` 语法只适用于单个对象的 new，不适用于 `new[]` 数组分配。  
数组初始化为 0 的正确写法是：
``` cpp
int *p = new int[40](); // 用空括号值初始化
int *p = new int[40]{}; // c++ 11 起用花括号初始化
```
...
## 枚举
枚举一般是在 C 语言程序设计考察，但是居然出现在了面向对象的模拟卷上，这里就展开介绍。  

首先分清两个概念：**枚举常量** 和 **枚举变量**：  
### 枚举常量
枚举常量就是在定义枚举类型时，在花括号内列出的那些标识符，默认情况下对应 0 开始的整数，且值不可修改。  
``` cpp
enum Color {
    RED,    // 枚举常量，值为 0
    GREEN,  // 值为 1
...
```
尽管这样，枚举常量的值是可以重复的，但是不建议。  

### 枚举变量
声明为某种枚举类型的变量，它的取值只能是该枚举类型中定义的枚举常量（或同类型的值）。  
``` cpp
enum Color favorite;          // favorite 就是一个枚举变量
favorite = BLUE;              // 正确，赋值为枚举常量
// favorite = 10;             // 虽然可能编译通过，但不推荐，类型不安全
```
枚举变量和普通变量一样，可以被赋值、修改、作为参数传递等，但为了代码清晰，通常只给它赋对应的枚举常量。  
...
枚举变量是可以像进行关系运算的。在 C++ 中不允许将整数隐式地赋值给枚举变量，必须显式转换。  
``` cpp
enum class Color { RED, GREEN, BLUE };
Color c = 0;                // 错误
Color c = static_cast<Color>(0); // 允许，但也不叫“直接”
```
