...
D. 没有用 `private`、`protected`、`public` 定义的数据成员是公有成员  

答案：C

C++ 没有规定 private、public 和 protected 的书写顺序，但是不能在类的声明中给数据成员赋初始值，数据成员的数据类型也不能是 register,没有用 private、protected 和 public 定义的数据成员是私有成员。  
...
在建立对象前可以为静态数据成员赋值，同时规定静态成员函数不能使用 this 指针，静态成员在类外定义时不需要使用 static 前缀，静态成员函数可以在类外也可以在类内定义。  
## 友元
### 友元函数
...
C. 友元函数必须在类的外部定义
D. 在外部定义为友元函数时，必须加关键字 `friend`

答案：A

C++ 中友元函数是独立于当前类的外部函数，一个友元函数可以同时定义为两个类的友元函数，友元函数既可以在类的内部，也可以在类的外部定义。在外部定义时，不必加 friend 关键字。  
### 友元类
一个类作为另一个类的友元
``` cpp
class Y{
